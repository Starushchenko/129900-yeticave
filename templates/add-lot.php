<form class="form form--add-lot container<?= ($errors) ? ' form--invalid' : '' ?>" action="/add.php" method="post"
    ...
    <div class="form__item form__item--wide<?= in_array('message', $errors) ? ' form__item--invalid' : '' ?>">
        ...
        <textarea id="message" name="message" placeholder="Напишите описание лота"><?= $lot_desc ?></textarea>
        <span class="form__error"><?= in_array('message',
                $errors) ? 'Описание лота обязательно для заполнения' : '' ?></span>
    </div>
    <div class="form__item form__item--file<?= in_array('photo', $errors) ? ' form__item--invalid' : '' ?>">
        <!-- form__item--uploaded -->
        ...
        <span class="form__error"><?= in_array('photo',
                $errors) ? 'Необходимо прикрепить jpg-изображение не более 500 Кб' : '' ?></span>
    </div>
